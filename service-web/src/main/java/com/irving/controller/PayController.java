...
import com.irving.common.Util;
import com.irving.common.WXConst;
...
public class PayController {
    ...
    public JSONObject wxPay(HttpServletRequest request){
        ...
        try{
             ...
             //MD5运算生成签名(MD5加密)，这里是第一次签名，用于调用统一下单接口
             String mysign = sign(prestr, WXConst.key, "utf-8").toUpperCase();
             ...
             //调用统一下单接口，并接受返回的结果
             String result = httpRequest(WXConst.pay_url, "POST", xml);


             System.out.println("调试模式_统一下单接口 返回XML数据：" + result);


             // 将解析结果存储在HashMap中(解析xml为hashMap)
             ...
             if(return_code == "SUCCESS" || return_code.equals(return_code)){
                 ...
                 String paySign = sign(stringSignTemp, WXConst.key, "utf-8").toUpperCase();
    ...
    public static void wxNotify(HttpServletRequest request,HttpServletResponse response) throws Exception{
        ...
        if("SUCCESS".equals(returnCode)){
            //验证签名是否正确
            if(verify(createLinkString(map), (String)map.get("sign"), WXConst.key, "utf-8")){
                /**此处添加自己的业务逻辑代码start**/
